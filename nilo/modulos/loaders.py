...
#@timeit 
def load_images_9(self, tile=None, Nx=9, Ny=9, Nb=3):
    ...
    for til in lista_tiles:
        print(til)
        path_tile_gx = f'{self}Images/{til}/individual_3.051arcsec/'
        ...
        for i,archive in enumerate(img):
            ...
            all_gx[i,:] = np.reshape(M_gx,[Npx])
            #all_gx[i,:] = np.concatenate([np.array(hdul[0].data),np.array(hdul[1].data),np.array(hdul[2].data)]).flatten(order='F')
...
def load_images_11(self, tile=None, Nx=11, Ny=11, Nb=3):
    """This method generates a list of dataframes for each tile 
    from the information contained in the images of galaxies and
    other objects.
    input: path to the folder containing the image folders for each tile.
    ...
    for til in lista_tiles:
        print(til)
        path_tile_gx = f'{self}Images/{til}/individual_3.729arcsec/'
        path_labels = f'{self}Galaxies/{til}_galaxies.csv'
        ...
        dec = np.zeros_like(ra)
        ID = np.zeros(len(img),dtype='<U24')
        TILE = np.zeros(len(img),dtype='<U4')
        label = np.zeros(len(img),dtype=int) 
...
def load_images_13(self, tile=None, Nx=13, Ny=13, Nb=3):
    """This method generates a list of dataframes for each tile 
    from the information contained in the images of galaxies and
    other objects.
    input: path to the folder containing the image folders for each tile.
    ...
    for til in lista_tiles:
        print(til)
        path_tile_gx = f'{self}Images/{til}/individual_4.407arcsec/'
        path_labels = f'{self}Galaxies/{til}_galaxies.csv'
...
def load_galaxy_table_all(folder="../../../Data/VVV/Galaxias_por_tile/", survey='VVV'):
    
    if survey not in ['VVV','VVVx']:
        print("Wrong survey name")
        return(None)
    
    if survey == 'VVV':
        pattern = "d???_galaxies.csv"
    else:
        pattern = "e????_galaxies.csv"
...
def load_extragalactic_table(filename, survey='VVV'):
    ...
    if survey == 'VVV':
        ...
        _data['TILE'] = _data['TILE'].str.decode(encoding = "ASCII")
...
def load_extragalactic_table_all(folder="../../../Data/VVV/Extragalactic/", survey='VVV'):
    ...
    if survey == 'VVV':
        pattern = "d???_extragalactic.fits"
    else:
        pattern = "e????_extragalactic.fits"
